...
bool Scene_Update(Scene *self)
{
	...
	if (!self->ui_mode) {
		...
		while (i < self->portalsCount)
		{
			...
			Portal_Update(portal);//TODO

			if (portal->state == CLOSE_PORTAL)
			{
				// Supprime le portail
				Scene_RemovePortal(self, i);
				removed = true;
			}
...
void Scene_RemovePortal(Scene* self, int index)
{
	Portal_Delete(self->portals[index]);//TODO
	Scene_RemoveObject(index, (void**)(self->portals), &(self->portalsCount));
}